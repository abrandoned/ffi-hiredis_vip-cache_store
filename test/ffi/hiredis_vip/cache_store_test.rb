...
describe FFI::HiredisVip::CacheStore do
  def setup
    @store  = FFI::HiredisVip::CacheStore::Store.new
    @dstore = FFI::HiredisVip::CacheStore::Store.new "redis://127.0.0.1:6379/5", "redis://127.0.0.1:6379/6"
    @pool_store  = FFI::HiredisVip::CacheStore::Store.new("redis://127.0.0.1:6379/2", pool_size: 5, pool_timeout: 10)
    @pool_store.connection_pool.class.must_equal ::ConnectionPool
    @pool_store.connection_pool.instance_variable_get(:@size).must_equal 5
  ...
  it "connects using an hash of options" do
    ...
    store.connection_pool.instance_variable_get(:@size).must_equal(5)
  ...
  describe "notifications" do
    ...
    # it "notifies on cleanup" # TODO implement in FFI::HiredisVip::CacheStore::Store
  ...
  private
    ...
    def with_notifications
      @events = [ ]
      FFI::HiredisVip::CacheStore::Store.instrument = true if instrument?
      ActiveSupport::Notifications.subscribe(/^cache_(.*)\.active_support$/) do |*args|
        @events << ActiveSupport::Notifications::Event.new(*args)
      end
      yield
    ensure
      FFI::HiredisVip::CacheStore::Store.instrument = false if instrument?
    end
